...
public class clsSubGroupTrip {
    ...
    clsSolution [] getTrip(clsGroupData groupdata,float Evaluation[]) throws IOException{
        ...
        clsTabooSearch TabooSearch=new clsTabooSearch();
         ArrayList<Integer> [] TouristClusters=SubGroups.getTouristClusters(groupdata);
    ...
    int getSocialRelationshipBetweenPersons(int Person1ID,int Person2ID, int Person[][]){
        int result;
    ...
    clsData [] getDataForTouristsSubgroups(clsGroupData groupdata,  ArrayList<Integer> [] TouristSubgroups ){
        ...
        for(int i=0;i<TouristSubgroups.length;i++){
            ...
            int MinimalBudgetLimitation=this.getMinimumBudgetLimitation(TouristSubgroups[i], groupdata.Person);
            int NumberOfVerticesOfTypeZ[]=this.getMinimumNumberOfVerticesOfTypeZ(groupdata.Person, TouristSubgroups[i]);
            clsData TuristSubgroupData=new clsData(groupdata.NumberOfTours,groupdata.NumberOfVertices,groupdata.NumberOfPersons,
                    ...
                    TuristSubgroupDataPOI);    
            result[i]=TuristSubgroupData;
        }
        return result;
    }
    int getMinimumBudgetLimitation(ArrayList<Integer> TouristSubgroup,int Person[][]){
        int Min=Person[TouristSubgroup.get(0)][1];
    ...
    int [] getMinimumNumberOfVerticesOfTypeZ( int Person[][],ArrayList<Integer> TouristSubgroup){
        int result[]=new int [10];
        for(int i=0;i<result.length;i++){
            int Min= Person[TouristSubgroup.get(0)][i+2+2*(Person.length-1)];
